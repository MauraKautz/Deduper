...
def parse_sam_line(line: str) -> tuple: #using tuple to store multiple items in a single variable, in an immutable storage method
    ...
    split_line = line.split() #shortcut for splitting the samline and calling an element
    umi = split_line[0].split(":")[-1] #splitting first element by ":" and pulling last element from new split line. this will be the last 8 characters/UMI
    chrom = split_line[2] #grabbing third element of samline, chromosome
    strand = find_strand(int(split_line[1])) #using function find_strand to determine if the read os on the + or - strand, turning the result into an integer and grabbing it from the second samline element
    pos = int(split_line[3]) #using function adjusted_pos to adjust 5' start position for clipping according to CIGAR string, grabbing true start position from fourth element in samline
    cigar = split_line[5]
...
with open(f, 'r') as input_file, open(o, 'w') as output_file: #opening input sam file for reading and output sam file for writing
    ...
    current_chrom = None #initializing chromosome as none to loop through each chromosome at a time
    chrom_dict = {}
